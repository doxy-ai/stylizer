...
namespace stylizer {
	...
	struct window {
		...
		inline bool should_close(bool process_events = true) const {
			if(process_events) glfw::pollEvents();
		...
		inline api::vec2u get_dimensions() const {
			api::vec2u out; std::tie(out.x, out.y) = raw.getSize();
			return out;
		}
		...
		window& reconfigure_surface_on_resize(context& ctx, api::surface::config config, stylizer::api::surface& surface) {
			raw.sizeEvent.append([&surface, &ctx, config](glfw::Window&, int x, int y) mutable {
				config.size = {static_cast<size_t>(x), static_cast<size_t>(y)};
				surface.configure(ctx, config);
			});
