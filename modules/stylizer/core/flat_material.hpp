...
float4 fragment(varryings vert) : SV_Target {
	...
	// return float4(231.0/255, 39.0/255, 37.0/255, 1.0) + sample * .001 + color * .001;
		...
		virtual std::span<maybe_owned<api::current_backend::buffer>> buffers(context& ctx) {
			return maybe_owned_span(span_from_value<api::current_backend::buffer>(config_buffer));
		}

		virtual std::span<std::string_view> requested_mesh_attributes() {
		...
		virtual std::span<api::current_backend::bind_group> bind_groups(context& ctx) {
