...
namespace stylizer {
    ...
    struct color_depth_geometry_buffer : public basic_geometry_buffer {
        texture color, depth;

        static color_depth_geometry_buffer create(context& ctx, const stdmath::vector<size_t, 3>& size, const std::optional<stdmath::vector<float, 4>>& clear_value, api::texture::format color_format, api::texture::format depth_format = api::texture::format::Depth_u24, api::texture::create_config config_override = {}) {
            ...
            color_depth_geometry_buffer out;
