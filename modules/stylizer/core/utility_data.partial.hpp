...
struct camera2D_gpu_data {
	float4x4 view_matrix; // at byte offset 0
	float4x4 projection_matrix; // at byte offset 64
	float3 offset; // at byte offset 128
	float3 target_position; // at byte offset 144
	...
	api::bool32 pixel_perfect = false; // at byte offset 172
...
static constexpr size_t minimum_utility_buffer_size = sizeof(time_gpu_data) + sizeof(camera3D_gpu_data) + sizeof(light_gpu_data);