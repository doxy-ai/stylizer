...
namespace stylizer {
	...
	struct single_texture_frame_buffer : public frame_buffer {
		maybe_owned<texture> texture;

		static single_texture_frame_buffer create(context& ctx, const stdmath::vector<size_t, 3>& size, const std::optional<stdmath::vector<float, 4>>& clear_value, api::texture::format color_format, api::texture::format depth_format = api::texture::format::Depth_u24, api::texture::create_config config_override = {}) {
			...
			out.texture = maybe_owned<stylizer::texture>::make_owned(texture::create(ctx, config));
			out.texture->configure_sampler(ctx, sample);
